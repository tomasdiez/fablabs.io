  ...
  task live_labs: :environment do
    ...
    labs_to_process.each_with_index do |lab_summary, index|
      ...
      if detail_response.is_a?(Net::HTTPSuccess)
        ...
        lab_attrs[:description] = detail_data['description'] if detail_data['description']
        lab_attrs[:email] = detail_data['email'] if detail_data['email']
        lab_attrs[:phone] = detail_data['phone'] if detail_data['phone']
        lab_attrs[:address_1] = detail_data['address_1'] if detail_data['address_1']
      end
      ...
      if lab.save(validate: false) # Skip strict validations like terms acceptance for seeds
        # Create some dummy links, machines, and projects to test the rich profile UI
        
        if lab.links.empty?
          lab.links.create(url: "https://example.com/#{slug}")
        end

        if lab.machines.empty?
          3.times do |i|
            machine = Machine.find_or_initialize_by(name: "Test Machine #{i} for #{slug}")
            machine.save(validate: false)
            lab.facilities.create(thing: machine)
          end
        end

        if lab.projects.empty?
          2.times do |i|
            project = Project.new(
              title: "Project #{i} by #{slug}",
              slug: "#{slug}-project-#{i}",
              description: "A cool project made at this lab.",
              workflow_state: 'published'
            )
            project.save(validate: false)
            # Add association through lab_projects if it exists, or directly
            lab.projects << project rescue nil 
          end
        end
